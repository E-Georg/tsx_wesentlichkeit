...
const isStakeholder = (item: Stakeholder): item is Stakeholder =>
  item && item.relevance !== undefined;

const EmailButtonList = ({
  stakeholders,
  subStakeholder,
  Betreff,
}: EmailButtonListProps) => {
  return (
    <>
      {stakeholders.map((item: Stakeholder) => {
        if (
          isStakeholder(item) &&
          subStakeholder.stakeholderId === item.id &&
          item.relevance === 1
        ) {
          return (
            <Emailbtn
              ...
              subject={Betreff}
              body={`http://localhost:5173/survey/${subStakeholder.id}`}
            />
          );
        }
        return null;
